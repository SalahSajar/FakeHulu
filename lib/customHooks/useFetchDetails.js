...
export const useFetchDetails = (showType) => {
    ...
    const fetchAccountHomePageShowsHandler = async () => {
        ...
        try{
            ...
            const trendingMoviesRes = await fetch(`https://api.themoviedb.org/3/trending/movie/day?api_key=${TMDb__API_KEY}`);
            const trendingTvShowsRes = await fetch(`https://api.themoviedb.org/3/trending/tv/day?api_key=${TMDb__API_KEY}`);
