...
export const useFetchDetails = (showType) => {
    ...
    const fetchShowDetailsHandler = async (showID) => {
        try{
            ...
            const similarShowsEndpoint =`https://api.themoviedb.org/3/${showType === "movie" ? "movie" : "tv"}/${showID}/recommendations?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY || process.env.NEXT_PUBLIC_VERCEL_TMDB_API_KEY}&language=en-US&page=1`;
