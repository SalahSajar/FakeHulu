...
export const useFetchDetails = (showType) => {
    ...
    const fetchShowDetailsHandler = async (showID) => {
        try{
            ...
            const showEndpoint =`https://api.themoviedb.org/3/${showType === "movie" ? "movie" : "tv"}/${showID}?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US`;
            const simularShowsEndpoint =`https://api.themoviedb.org/3/${showType === "movie" ? "movie" : "tv"}/${showID}/recommendations?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US&page=1`;
            const showVideosEndPoint = `https://api.themoviedb.org/3/${showType === "movie" ? "movie" : "tv"}/${showID}/videos?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US`
    ...
    const fetchActorDetailsHandler = async (actorID) => {
        try{
            ...
            const actorDetailsRes = await fetch(`https://api.themoviedb.org/3/person/${actorID}?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US`);
            const actorMoviesRes = await fetch(`https://api.themoviedb.org/3/person/${actorID}/movie_credits?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US`);
            const actorTvShowsRes = await fetch(`https://api.themoviedb.org/3/person/${actorID}/tv_credits?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US`);
            const actorSocialMediaRes = await fetch(`https://api.themoviedb.org/3/person/${actorID}/external_ids?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US`);
    ...
    const fetchAccountHomePageShowsHandler = async () => {
        ...
        try{
            ...
            const bestTvShowsOf2021Res = await fetch(
                `https://api.themoviedb.org/3/discover/tv?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&first_air_date_year=2021&page=1&include_null_first_air_dates=false&with_watch_monetization_types=flatrate&with_status=0&with_type=0`
            );
