...
export const useFetchDetails = (showType) => {
    ...
    const TMDb__API_KEY = InDevInv ? process.env.NEXT_PUBLIC__TMDB__API_KEY : process.env.NEXT_PUBLIC_VERCEL_TMDB__API_KEY;
    ...
    const fetchAccountHomePageShowsHandler = async () => {
        ...
        try{
            ...
            const topRatedMoviesRes = await fetch(
                `https://api.themoviedb.org/3/discover/movie?api_key=${TMDb__API_KEY}&include_adult=false&language=en-US&page=1&sort_by=vote_average.desc&vote_count.gte=200`
            );
            ...
            const topRatedTvShowsRes = await fetch(
                ...
                // `https://api.themoviedb.org/3/discover/tv?api_key=${TMDb__API_KEY}&include_adult=false&language=en-US&page=1&sort_by=vote_average.desc&vote_count.gte=200`
            );
