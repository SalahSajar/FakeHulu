...
const GenresContextProvider = ({children}) => {
    ...
    const fetchGenresHandler = async () => {
        const movieGenresRes = await fetch(
            `https://api.themoviedb.org/3/genre/movie/list?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY || process.env.NEXT_PUBLIC_VERCEL_TMDB_API_KEY}&language=en-US`
        );
        const tvShowGenresRes = await fetch(
            `https://api.themoviedb.org/3/genre/tv/list?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY || process.env.NEXT_PUBLIC_VERCEL_TMDB_API_KEY}&language=en-US`
        );
