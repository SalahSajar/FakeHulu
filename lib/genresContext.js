...
const GenresContextProvider = ({children}) => {
    ...
    const fetchGenresHandler = async () => {
        const movieGenresRes = await fetch(
            `https://api.themoviedb.org/3/genre/movie/list?api_key=${TMDB__API_KEY}&language=en-US`
        );
        const tvShowGenresRes = await fetch(
            `https://api.themoviedb.org/3/genre/tv/list?api_key=${TMDB__API_KEY}&language=en-US`
        );
