...
const SearchContextProvider = ({children}) => {
  ...
  const searchMostPapularActors = async () =>{
    try{
      ...
      const popularActorsRes = await fetch(`https://api.themoviedb.org/3/person/popular?api_key=${TMDB__API_KEY}&language=en-US&page=1`);
    ...
    const popularActors = await popularActorsRes.json();
      ...
      setActorsFounded(() => {
        return popularActors.results.filter(
          (popularActor) => popularActor.known_for_department && popularActor.known_for_department.toLowerCase() === "acting"
        )
      });
