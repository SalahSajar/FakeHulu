...
const SearchContextProvider = ({children}) => {
    ...
    try{
      ...
      const searchRes = await fetch(`https://api.themoviedb.org/3/search/${type}?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US&page=1&query=${searchValue}&include_adult=false`);
  ...
  const searchMostPapularActors = async () =>{
    try{
      ...
      const popularActorsRes = await fetch(`https://api.themoviedb.org/3/person/popular?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US&page=1`);
  ...
  const searchTrendingShows = async (type) =>{
    try{
      ...
      const trendingShowsRes = await fetch(`https://api.themoviedb.org/3/trending/${type}/week?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}`);
